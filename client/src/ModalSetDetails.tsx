...
import { computeProgressionByBenefitsAndEra, computeProgressionByMemberAndEra } from './helpers';
...
export const ModalSetDetails = ({ album, category, isModalSetDetailsOpen, setIsModalSetDetailsOpen, progression }: IModalSetDetails) => {
    ...
    if (currentAlbum != album.name && album.name.length > 0) {
        ...
        const newCards = cardsFile.filter(card => card.era === album.name && card.categories.includes(category));
        const progressionMembers = computeProgressionByMemberAndEra(cardsData, newCards);
        const progressionBenefits = computeProgressionByBenefitsAndEra(cardsData, newCards);
        newBenefits = Object.entries(progressionBenefits).map(([key, value]) => ({ benefit: key, progression: value }));
        newMembers = Object.entries(progressionMembers).map(([key, value]) => ({ member: key, progression: value }));
    ...
    const handleClickOnLink = (type: string, value: string) => {
        ...
        let newBenefits = benefitsFile.map(_benefit => currentBenefitsName.includes(_benefit.name) ? { ..._benefit, display: true, checked: true } : { ..._benefit, display: false, checked: false });
        let newMembers = membersFile.map(_member => currentMembersName.includes(_member.name) ? { ..._member, display: true, checked: true } : { ..._member, display: false, checked: false });
        switch (type) {
            case "benefit":
                newBenefits = newBenefits.map(_benefit => _benefit.name === value ? { ..._benefit, checked: true } : { ..._benefit, checked: false });
                break;
            case "member": ;
                newMembers = newMembers.map(_member => _member.name === value ? { ..._member, checked: true } : { ..._member, checked: false });
                break;
            default:
                break;
        }
                ...
                <div className="modal-set-details-container__body__header">
                    ...
                    <div className="modal-set-details-container__body__header__owned-resume">
                        <div className="modal-set-details-container__body__header__owned-resume-box">
                            <span>{progression[album.name] ? progression[album.name].acquired : 0}</span>
                    ...
                    <div className="modal-set-details-container__body__main__benefit-progress">
                        ...
                        {benefits.map(object => {
                            return (
                                ...
                                replace>
                                    {object.benefit}
                                    <StatsInfo album={{ name: object.benefit, code: object.benefit }} progression={object.progression} />
                                </Link>
                            )
                        })}
                    </div>
                    <div className="modal-set-details-container__body__main__member-progress">
                        ...
                        {members.map(object => {
                            return (
                                ...
                                replace>
                                    {object.member}
                                    <StatsInfo album={{ name: object.member, code: object.member }} progression={object.progression} />
                                </Link>
                            )
                        })}
                    </div>
