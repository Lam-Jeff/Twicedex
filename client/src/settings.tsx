...
export const Settings = () => {
  const [membersSettings, setMembersSettings] = useState(
    members.map((member) => ({ name: member.name, checked: true })),
  );
  ...
  useEffect(() => {
    const preferencesMembers = sessionStorage.getItem("preferences-members");
    if (preferencesMembers) {
      const members: { name: string; checked: boolean }[] =
        JSON.parse(preferencesMembers);
  ...
  const handleMembersSettings = (index: number) => {
    const newArray = membersSettings.map((object, currentIndex) =>
  ...
  const handleSelectAllMembers = () => {
    if (!selectAllMembers) {
      const newArray = membersSettings.map((object) => ({
        ...object,
        checked: true,
      }));
      setMembersSettings(newArray);
  ...
  const handleSaveSettings = () => {
    sessionStorage.setItem(
      "preferences-members",
      JSON.stringify(membersSettings),
    );
    ...
    <div className="settings-container">
      ...
      <div className="settings-container__stats-box">
        <div className="settings-container__stats-box__members">
          ...
          <ul>
            <li>
              <label htmlFor={`member-ot9`} key={`label-member-ot9`}>
            ...
            {membersSettings.map((object, index) => {
              return (
                <li key={`list-element-member-${object.name}`}>
                  <label
                    htmlFor={`member-${object.name}`}
        ...
        <button
          ...
          aria-label={`Save preferences and go back to previous page`}
        >
          SAVE
        </button>
      </div>
