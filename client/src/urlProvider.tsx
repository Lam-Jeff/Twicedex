...
import { generatePath, SetURLSearchParams, useLocation, useSearchParams } from "react-router-dom";
...
interface IUrlProviderProps {
    ...
    */
     ...
     */
    computeNewPath: (category: string, code: string, cardID: string | undefined, display: string) => void,

    /**
     * Update parameters in Url and local storage.
     */
    updateParams: (benefits: { name: string, checked: boolean, display: boolean }[], members: { name: string, checked: boolean, display: boolean }[]) => void
...
export const UrlProvider = ({ children }: Props) => {
    ...
    const [categoryUrl, setCategoryUrl] = useState<string>(categoryLocalStorage ? JSON.parse(categoryLocalStorage) : global.CATEGORY_DEFAULT_VALUE);
    const [codeUrl, setCodeUrl] = useState<string>(codeLocalStorage ? JSON.parse(codeLocalStorage) : global.ERA_DEFAULT_VALUE);
    const [displayUrl, setDisplayUrl] = useState<string>(displayLocalStorage ? JSON.parse(displayLocalStorage) : global.DISPLAY_DEFAULT_VALUE);
    ...
    */
    const resetUrl = () => {
        ...
        localStorage.setItem('benefits', JSON.stringify(global.BENEFITS_DEFAULT_VALUE));
        localStorage.setItem('members', JSON.stringify(global.MEMBERS_DEFAULT_VALUE));
        localStorage.setItem('display', JSON.stringify(global.DISPLAY_DEFAULT_VALUE));
    }
    ...
    */
    const updateParams = (benefits: { name: string, checked: boolean, display: boolean }[], members: { name: string, checked: boolean, display: boolean }[]) => {
        const benefitsStringify = JSON.stringify(benefits.filter(benefit => benefit.checked).map(benefit => benefit.name));
        const membersStringify = JSON.stringify(members.filter(member => member.checked).map(member => member.name));
    ...
    */
    const computeNewPath = (category: string, code: string, cardID: string | undefined = undefined, display: string) => {
    ...
    return <UrlContext.Provider value={authValue}>{children}</UrlContext.Provider>;
};
